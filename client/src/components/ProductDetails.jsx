...
class ProductDetails extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      prevProps.styleIndex !== this.props.styleIndex ||
      prevProps.MAWproductData !== this.props.MAWproductData
    ) {
  ...
  changeSize(e) {
    let sku = this.props.MAWstylesData.results[this.props.styleIndex].skus[
      e.target.value
    ];
    let quantityArray = [];
  ...
  render() {
    if (
      this.props.MAWproductData === undefined ||
      this.props.MAWavgRating === undefined
    ) {
      return <div>Loading...</div>;
    } else {
      let original_price, sale_price;
      if (
        this.props.MAWstylesData.results[this.props.styleIndex].sale_price > 0
      ) {
        original_price = (
        ...
        sale_price = (
          <div className="ml-3" style={{ color: "red" }}>
            {" "}
            $
            {this.props.MAWstylesData.results[this.props.styleIndex].sale_price}
          </div>
        );
      } else {
        original_price = (
      ...
      let styleArray = this.props.MAWstylesData.results.map((styleObj) => {
        ...
        if (
          ...
          this.props.MAWstylesData.results.indexOf(styleObj)
        ) {
          borderStyle = {
            width: 60,
            height: 60,
        ...
        return (
          <img
            ...
            onClick={(e) => this.changeStyleIndex(e)}
          />
        );
      });
      ...
      let sizesSelector =
        Object.keys(
          this.props.MAWstylesData.results[this.props.styleIndex].skus
        )[0] !== "null" ? (
          <select
            className="mr-3"
          ...
          <div className="mr-3" style={{ color: "red" }}>
            Item Out Of Stock
          </div>
        );
      ...
      if (this.state.addedToCart === false) {
        addToCartButton =
          this.state.selectedSize !== "Select Size" ? (
            <button className="addToCart" onClick={(e) => this.addToCart(e)}>
              Add To Cart
            </button>
          ) : (
            <div></div>
        ...
        addToCartButton =
          this.state.selectedSize !== "Select Size" ? (
            <div>
              ...
              <div style={{ color: "red" }}> Added To Cart!</div>
            </div>
          ) : (
            <div></div>
          );
      ...
      return (
        <div>
          <StarRatings
            rating={this.props.MAWavgRating}
          ...
          <div>
            <div>
              <b>Style: </b>
              {this.props.MAWstylesData.results[this.props.styleIndex].name}
            </div>
          </div>
          <Container>
            <Row>{mappedStyleThumbnails}</Row>
          </Container>
          <Container>
            ...
            <Row className="mt-3 mb-3">{addToCartButton}</Row>
